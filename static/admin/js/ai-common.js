// Глобальные переменные
            ...
            function resolveClientId() {
                ...
                const sessionId = sessionIdFromMeta || getCookieValue('DLSID');
        ...
        function setLanguageUrlParam(language) {
            try {
                const url = new URL(window.location.href);
                url.searchParams.set('ui_language', language);
            ...
            document.getElementById("selectLang").addEventListener("change", async function () {
                ...
                // Persist selected language in the URL so switching between chat pages keeps it.
                setLanguageUrlParam(selectedLang);
            ...
            document.addEventListener("DOMContentLoaded", async () => {
                ...
                async function fetchProblemData() {
                    ...
                    try {
                        const response = await fetch(getProblemDataUrl());
                ...
                function populatePrompts(languageId, topicId) {
                    ...
                    let regularPrompts = (problemData.prompts || [])
                        .filter(p => p.topic_id !== null && p.topic_id !== undefined && p.topic_id !== '')
                        .slice();
                    ...
                    const filteredPrompts = filterPrompts(regularPrompts, languageId, topicId);
