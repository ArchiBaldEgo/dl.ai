...
│   ├── modules/
│   │   ├── auth.js      # вход на DeepSeek через «Войти с помощью Google» + проверка сохранённой сессии
...
| `BOT_USERNAME` | — | Email аккаунта **Google**, к которому привязан DeepSeek (**обязательно**) |
| `BOT_PASSWORD` | — | Пароль от этого Google-аккаунта (**обязательно**) |
| `BOT_PROFILE_DIR` | `WebDeepseek/worker/.chrome-profiles` | Каталог постоянных Chrome-профилей (по одному на слот бота). Хранит сессию Google, чтобы не логиниться заново |
...
## Авторизация: «Войти с помощью Google»

На сайте DeepSeek **нет** обычных полей логина/пароля — вход выполняется только кнопкой
«Войти с помощью Google» (Google OAuth). Поэтому `BOT_USERNAME`/`BOT_PASSWORD` — это реквизиты
**Google-аккаунта** (email + пароль), к которому привязан DeepSeek.

Поток входа (`worker/modules/auth.js`):

1. Открыть `https://chat.deepseek.com/sign_in`.
2. Нажать «Войти с помощью Google».
3. Дождаться страницы `accounts.google.com` (popup или переход в той же вкладке).
4. Ввести email → «Далее».
5. Ввести пароль → «Далее».
6. При необходимости подтвердить доступ («Разрешить»/«Allow»).
7. Дождаться возврата на `chat.deepseek.com` и появления поля чата.

### Постоянные профили (важно)

Каждый бот запускает Chrome с собственным `userDataDir`
(`WebDeepseek/worker/.chrome-profiles/bot-<id>`, переопределяется через `BOT_PROFILE_DIR`).
Профиль хранит сессию Google, поэтому:

- при перезапуске бота / контейнера логин обычно **не требуется** — сессия подтягивается из профиля;
- slot-id бота стабилен (`1..MAX_BOT_COUNT`, переиспользуется), поэтому профиль привязан к слоту;
- Google получает минимум поводов для срабатывания защиты от автоматизации — OAuth-флоу
  выполняется один раз на профиль, дальше работает «быстрый путь» (`checkAlreadyAuthorized`).

> Каталог профилей добавлен в `.gitignore` — он содержит сессию Google и не должен попадать в git.

### Если Google блокирует автоматический вход

Google может отклонить headless-вход с ошибкой «This browser or app may not be secure». В этом
слоге пула бот переходит в состояние `not_autorized` (HTTP 401 от API). Надёжный обход —
**одноразовый ручной log-in** для засева профиля:

1. Временно выставить `HEADLESS=false` (видимый Chrome) и пробросить дисплей (X11/VNC) в контейнер.
2. Для каждого слота `1..MAX_BOT_COUNT` один раз пройти Google-вход руками — сессия сохранится в
   `WebDeepseek/worker/.chrome-profiles/bot-<id>`.
3. Вернуть `HEADLESS=true`. Дальше боты стартуют по «быстрому пути» без повторного OAuth.

При смене пароля Google-аккаунта засев профилей нужно повторить.
