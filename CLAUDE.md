...
- **Single source of truth (DRY).** Do not copy an existing helper — import it:
  ...
  - cache of passed task solutions → `ai/services/solution_cache.py` (`TaskSolution`):
    ...
    (`send_solution_view`, HTTP), OR from batch-solve ARM when the operator checked
    «Сохранять решённые задачи» (`record_batch_solution`: только verdict solved; уже решённые
    (node+язык) не трогаются; сбои кэша прогон не ломают; флаг `saved_solution` — только в live-item'ах,
    в DB-rebuild снапшотах его нет); a passed solution is served
    ...
    `course_id` (from send-solution) is stored too and powers the admin
    «Решённые задачи» listing (`TaskSolutionAdmin`, custom
    `tasksolution_changelist.html`): localized date, task name → user DL link,
    language NAME via `ProgrammingLanguage`, the user's chosen prompt; row click
    (outside the task link) expands the saved code. Testing is logged to
    `AIRequestLog` with `mode=testing` / `source=http` and
...
- `serializers.py` / `i18n.py` — lightweight serializers + UI-language localization (`name_ru/en/fr`); `get_language_instruction(ui_language)` appends a non-Russian reply constraint for every non-Russian message. Gotcha: `get_localized_name` при пустых `*_ru` полях НИКОГДА не возвращает `str(obj)` — `__str__` Prompt/Topic/SharedPrompt делегируют в этот же хелпер, и старый фолбэк зацикливался (RecursionError → 500 на `/admin/ai/prompt/add/`); фолбэк — безопасная подпись `Класс #pk` (без pk — просто класс).
...
- `arm_runner.py` — async ARM runner; in-memory live job, but DB (`AIModelTestRun`/`AIModelTestResult`, `run_type` single/batch) is the source of truth (`get_arm_run_snapshot` falls back to it; `light_results=True` — batch-результаты без тяжёлого тела: raw_response/code/short_response/dl_comment/dl_error заменяются флагом `lazy_body` — страница /arm/solve/, поллинг статуса и снапшот детали журнала не таскают десятки МБ текста, тело каждой записи подтягивается по AJAX при раскрытии строки через `/ai/admin/arm/solve/result/<id>/body/` (`admin_arm_solve_result_body_view`); полный снапшот — для XLSX-экспорта). Мультиязычность и мультипроцессы /arm/solve/ (2026-09): языки на форме — ЧЕКБОКСЫ, на каждый выбранный язык стартует ОТДЕЛЬНЫЙ batch-прогон (одни node_ids/модели) — констрейнт `(run, model_key, task)` запрещает смешивать языки в одном прогоне; страница ведёт список «Процессы» (armSolveProcessesCard) с параллельными поллерами по run_id, кнопками «Показать»/«Прервать» и фокусом таблицы (`focusedRunId`); форма НЕ блокируется на время прогонов, run_name при нескольких языках суффиксится расширением (`.i86`/`.mpc`); тема-селект активна только при ровно одном языке (в журнал), препромпт-привязки всё равно резолвятся per-task на язык каждой задачи; тема-селект показывает ОБЪЕДИНЕНИЕ тем выбранных языков, подпись привязок — построчно по каждому языку). Отчёт «Сводка» (report, `_build_batch_report`): models_count/tasks_count (объём), errors_by_topic (failed+skipped по темам, кап 6 позиций/тема, остаток hidden), run_meta (время — `_attach_run_meta`: старт/финиш/длительность; attach в worker finalize, `_snapshot_from_test_run`, `logs._build_batch_log_snapshot`). Uses `EXTENSION_TO_LANG` from `task_registry` for the language-name fallback. The manual run name is stored on `AIModelTestRun.run_name` (legacy fallback: `run_params["run_name"]`) and read via `ai/admin/logs.py::run_name_for` — «Настройка ИИ-приложения» has no run-name field anymore. Code extraction (`_extract_code_from_response`): longest ``` fence wins; unfenced text is accepted only when it looks like code (`_looks_like_code`), and think-blocks are stripped server-side (`_strip_think_blocks`) before anything is stored/extracted. In batch-solve, an unset (or nonexistent) `prompt_id` means «по привязке»: the worker resolves `ArmPromptBinding` PER TASK from the task's DL branch/topic (`_resolve_batch_prompt`, topic guessed from the `path` segments against `Topic.topic_name_ru`) — the start view no longer resolves a global binding. Gotcha: the binding language is the run form's selected language (`fallback_language_id` wins over `task.programming_language_id` — dual-language courses like «[Ассемблер i8086, C-MPA]» reuse the same DL tree nodes, and a Task's stored language can be stale from a previous run in the other language). The batch worker passes `programming_language_id` into `ensure_task`, which re-guesses a foreign-language topic from the DL path scoped to the run language (`_guess_topic_from_path(path, programming_language_id=…)`; topics are per-language — two Topics can share a name across the course's two languages), so the `(new_language, stale_topic)` mismatch that left tasks without a preprompt is resolved. If the topic name is absent in the run language, the stale topic is reset to NULL and binding resolution falls back to the language-wide binding.
...
- Django-шаблоны: комментарий `{# … #}` допустим ТОЛЬКО в одну строку — многострочный
  ...
  он, в свою очередь, должен идти ПОСЛЕ `{% extends %}` (extends обязан быть первым тегом).
  Регрессия: `TemplateInlineCommentTests` (ai/tests.py) сканирует все `ai/templates/**/*.html`.
- Скорость changelist'ов: во всех ModelAdmin с `list_filter` стоит
  `show_facets = admin.ShowFacets.NEVER` («Show counts» = COUNT(*) на каждый вариант
  фильтра — при больших таблицах это заметно тормозит загрузку страницы). Не возвращать.
