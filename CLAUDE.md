...
- `admin/` — custom `ai_admin_site` at `/ai/admin/`: `site.py` (`AIAdminSite`, `/ai/admin/ai/`→`/ai/admin/` redirect; `each_context` injects AI tool groups into `available_apps` — закреплённый пункт «Настройка ИИ-приложения» (группа `ai-pinned`, без заголовка, из «Раздела ИИ» убирается) наверху, затем grouped Промпты→ARM→Диагностика→Система with icons/hints, `_HIDDEN_NAV_OBJECT_NAMES` hides sections from nav but keeps direct URLs), `models.py` (ModelAdmins + «Автоперевод» action; `AIAppSettingsAdmin` change form embeds «последние 5 запросов» via `logs.build_recent_log_rows` + «последние 5 пакетных решений» via `logs.build_recent_batch_rows` (клик по строке разворачивает таблицу результатов прогона — тот же `window.ArmBatchResults`, что на /arm/solve/)), `forms.py`, `arm.py` (ARM views), `my_prompt.py`, `logs.py` (журнал + `q`/`id` search filters + recent-logs helper), `model_status.py`, `updates.py` (commit-history page), `prompt_regression.py`, `test_console` views, `active_runs.py` (личное меню «мои процессы» в шапке админки: свои прогоны всех раннеров, включая завершённые за 10 минут — фильтр по `user_id` из job-ов, НЕ суперпользовательский список; плюс глобальная безличная запись обновления моделей из `model_health.list_model_refresh_runs`), `auth.py`/`permissions.py`. Core permission logic in `site.py`. Left nav renders through the PROJECT override `ai/templates/admin/app_list.html` (iconised tool groups above real model apps; `static/admin/js/ai_nav_filter.js` extends the stock quick filter to them).
